    ...
    <table class="table mt-5">
        ...
        <tbody>
            @foreach ($books as $book)
                <tr>
                    <td>{{ $book->name }}</td>
                    <td>{{ $book->author->name_surname }}</td>
                    <td>{{ $book->publisher->name }}</td>
                    <td>{{ $book->user->name }}</td>
                    <td>
                        <button type="button" class="btn btn-success open-modal" data-toggle="modal" data-target="#myModal"
                            data-book-id="{{ $book->id }}" data-book-name="{{ $book->name }}" title="Add Comment">
                            <i class="fa fa-comment"></i>
                        </button>


                        <a href="{{ route('book.comments', $book->id) }}" class="btn btn-info" title="Book Comments">
                            <i class="fa fa-eye"></i>
                        </a>
                    </td>
                </tr>
