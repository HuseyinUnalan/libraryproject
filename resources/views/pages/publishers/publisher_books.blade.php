...
<div class="container mt-5">
    <h2> Published Books by '{{ $publisher->name }}' </h2>
    <table class="table mt-5">
        ...
        <tbody>
            @foreach ($publisherbooks as $publisherbook)
                <tr>
                    <td>{{ $publisherbook->name }}</td>
                    <td>{{ $publisherbook->author->name_surname }}</td>
                    <td>{{ $publisherbook->publisher->name }}</td>
                    <td>{{ $publisherbook->user->name }}</td>
                    <td>
                        <button type="button" class="btn btn-success open-modal" data-toggle="modal" data-target="#myModal"
                            data-book-id="{{ $publisherbook->id }}" data-book-name="{{ $publisherbook->name }}"
                            title="Add Comment">
                            <i class="fa fa-comment"></i>
                        </button>


                        <a href="{{ route('book.comments', $publisherbook->id) }}" class="btn btn-info"
                            title="Book Comments">
                            <i class="fa fa-eye"></i>
                        </a>
                    </td>
